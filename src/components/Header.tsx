      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-18">
          ...
          <div className="flex items-center gap-3 animate-fade-in-left">
            ...
            <div>
              <span className="text-2xl font-black bg-gradient-to-r from-slate-900 to-slate-700 bg-clip-text text-transparent">Biosoft</span>
              <div className="text-xs text-slate-500 font-medium -mt-1 tracking-wider">HEALTHCARE MADE SIMPLE</div>
            </div>
          </div>
          ...
          {/* Mobile Menu Button */}
          <button className="md:hidden p-3 hover:bg-slate-100 rounded-xl transition-colors duration-200" onClick={() => setIsMenuOpen(!isMenuOpen)}>
        ...
        <div
          className={`md:hidden absolute top-18 left-0 right-0 bg-white/95 backdrop-blur-xl border-b border-slate-200/50 shadow-xl transition-all duration-300 ${
          ...
          <div className="px-4 py-8 space-y-6">
            {navigation.map((item, index) => (
              <a
                key={item.name}
                href={item.href}
                className={`block text-slate-700 hover:text-blue-600 transition-all duration-300 font-semibold py-3 transform text-lg ${
            ...
            <div
              className={`pt-6 space-y-4 transform transition-all duration-500 ${isMenuOpen ? "translate-x-0 opacity-100" : "-translate-x-4 opacity-0"}`}
